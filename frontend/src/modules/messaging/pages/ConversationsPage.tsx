...
export const ConversationsPage: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto h-[calc(100vh-64px)] flex border-x bg-white">
      {/* Sidebar */}
      <div
        className={`w-full md:w-80 flex-shrink-0 border-r flex flex-col ${conversationId ? "hidden md:flex" : "flex"}`}
      >
      ...
      {/* Content */}
      <div
        className={`flex-1 flex flex-col min-w-0 ${!conversationId ? "hidden md:flex" : "flex"}`}
      >
        {!conversationId ? (
          <EmptyInbox />
        ) : (
